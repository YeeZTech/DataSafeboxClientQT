...
public:
    ...
    Q_INVOKABLE QString apiBaseUrl()           const { return QLatin1String(AppCfg::API_BASE_URL); }
    Q_INVOKABLE QString soketiWsHost()         const { return QLatin1String(AppCfg::SOKETI_WS_HOST); }
    Q_INVOKABLE QString soketiAppKey()         const { return QLatin1String(AppCfg::SOKETI_APP_KEY); }
    Q_INVOKABLE QString casdoorEndpoint()      const { return QLatin1String(AppCfg::CASDOOR_ENDPOINT); }
    Q_INVOKABLE QString casdoorClientId()      const { return QLatin1String(AppCfg::CASDOOR_CLIENT_ID); }
    Q_INVOKABLE QString casdoorRedirectUri()   const { return QLatin1String(AppCfg::CASDOOR_REDIRECT_URI); }
    Q_INVOKABLE QString websiteUrl()           const { return QLatin1String(AppCfg::WEBSITE_URL); }
    Q_INVOKABLE QString walletUrl()            const { return QLatin1String(AppCfg::WALLET_URL); }
    Q_INVOKABLE QString userCenterUrl()        const { return QLatin1String(AppCfg::USER_CENTER_URL); }
    Q_INVOKABLE QString vendorUrl()            const { return QLatin1String(AppCfg::VENDOR_URL); }
    Q_INVOKABLE QString vendorCompanyName()    const { return QString::fromUtf8(AppCfg::VENDOR_COMPANY_NAME); }
    Q_INVOKABLE QString helpDocsUrl()          const { return QLatin1String(AppCfg::HELP_DOCS_URL); }
    Q_INVOKABLE QString customerServiceUrl()   const { return QLatin1String(AppCfg::CUSTOMER_SERVICE_URL); }
    Q_INVOKABLE QString customerServiceToken() const { return QLatin1String(AppCfg::CUSTOMER_SERVICE_TOKEN); }
    ...
    // 分页配置访问方法
    Q_INVOKABLE int securityDomainPageSize()   const { return AppCfg::SECURITY_DOMAIN_PAGE_SIZE; }
    Q_INVOKABLE int visibleUsersPageSize()     const { return AppCfg::VISIBLE_USERS_PAGE_SIZE; }
    Q_INVOKABLE int messageCenterPageSize()    const { return AppCfg::MESSAGE_CENTER_PAGE_SIZE; }
    
    // 详情页和缓存配置访问方法
    Q_INVOKABLE int detailDialogMaxWidth()     const { return AppCfg::DETAIL_DIALOG_MAX_WIDTH; }
    Q_INVOKABLE int detailDialogMaxHeight()    const { return AppCfg::DETAIL_DIALOG_MAX_HEIGHT; }
    Q_INVOKABLE int detailCacheMaxSize()       const { return AppCfg::DETAIL_CACHE_MAX_SIZE; }
    Q_INVOKABLE int detailCacheTtlSeconds()    const { return AppCfg::DETAIL_CACHE_TTL_SECONDS; }
};
