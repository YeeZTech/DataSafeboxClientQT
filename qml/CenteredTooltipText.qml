...
Item {
    ...
    function _fixedMiddleTruncate(s, before, after) {
        if (_charUnits(s) <= before + after) return s
        var w = 0, i = 0
        while (i < s.length) {
            var cw = s.charCodeAt(i) > 127 ? 2 : 1
            if (w + cw > before) break
            w += cw; i++
        }
    ...
    readonly property string _displayText: _useFixedTruncate
        ? _fixedMiddleTruncate(value, beforeChars, afterChars)
        : value
    readonly property bool truncated: value.length > 0 && (
        _useFixedTruncate
            ? _charUnits(value) > beforeChars + afterChars
            : fm.advanceWidth(value) > _availWidth
    )
    ...
    Component {
        ...
        Item {
            ...
            readonly property real bubbleXInWin: Math.max(
                _minX,
                Math.min(_idealX, Math.max(_minX, _maxX))
            )

            // 三角 X（相对 overlay）：对准省略号中心，clamp 在气泡两端
            readonly property real arrowX: Math.max(4, Math.min(
                anchorX - bubbleXInWin - arrowSize,
                bubbleW - arrowSize * 2 - 4
            ))
            ...
            x: bubbleXInWin
            y: (flipDown
               ? (cellTopLeft.y + root.height)
               : (cellTopLeft.y - bubbleH - arrowSize)) + 5
