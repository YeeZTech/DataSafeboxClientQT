...
Item {
    ...
    function _isShown() {
        ...
        try {
            _db().transaction(function (tx) {
                ...
                if (rs.rows.length > 0 && rs.rows.item(0).value === "1")
                    result = true;
            });
        } catch (e) {
    ...
    function _updateBubble() {
        ...
        if (typeof dimMask !== "undefined" && dimMask)
            dimMask.requestPaint();
    }
    ...
    Item {
        ...
        Canvas {
            ...
            Connections {
                ...
                function on_SpotXChanged() {
                    dimMask.requestPaint();
                }
                function on_SpotYChanged() {
                    dimMask.requestPaint();
                }
                function on_SpotWChanged() {
                    dimMask.requestPaint();
                }
                function on_SpotHChanged() {
                    dimMask.requestPaint();
                }
            }
                ...
                Item {
                    ...
                    Row {
                        ...
                        Rectangle {
                            ...
                            radius: 3
                            anchors.verticalCenter: parent.verticalCenter
                            color: root._neverShowAgain ? (cbMouseArea.pressed ? "#3a8fc7" : cbMouseArea.containsMouse ? "#5ab0e8" : "#4ca3e0") : cbMouseArea.pressed ? Qt.rgba(1, 1, 1, 0.15) : cbMouseArea.containsMouse ? Qt.rgba(1, 1, 1, 0.08) : "transparent"
                            border.color: root._neverShowAgain ? "transparent" : cbMouseArea.containsMouse ? Qt.rgba(1, 1, 1, 0.75) : Qt.rgba(1, 1, 1, 0.4)
                            border.width: 1
                            Behavior on color {
                                ColorAnimation {
                                    duration: 120
                    ...
                    Rectangle {
                        ...
                        height: 26
                        radius: 6
                        color: prevBtnArea.pressed ? Qt.rgba(1, 1, 1, 0.15) : prevBtnArea.containsMouse ? Qt.rgba(1, 1, 1, 0.08) : "transparent"
