...
Item {
    ...
    function toJsArray(value) { return DomainUtils.toJsArray(value) }
    function normalizeInstances(instances) { return DomainUtils.normalizeInstances(instances) }
    function sortByTimeDesc(list, p, s) { return DateTimeUtils.sortByTimeDesc(DomainUtils.toJsArray(list), p, s) }
    function dedupeWhitelistAudits(list) { return DomainUtils.dedupeWhitelistAudits(list) }
    function dedupeExportAudits(list) { return DomainUtils.dedupeExportAudits(list) }
    ...
    function resolveWhitelistProcessesByRow(row) { return DomainUtils.resolveWhitelistProcessesByRow(row) }
    ...
    function formatPayerText(payerValue) {
        ...
        if (payerValue === "创建方" || payerValue === "安全域创建方" || payerValue === "创建者") return qsTr("Creator")
        if (payerValue === "使用方" || payerValue === "安全域使用方" || payerValue === "使用者") return qsTr("User")
        return payerValue
    }
